...
def generate_name(business_domain_industry):
    try:
        ...
        if not 200 <= response.status_code < 300:
            print("Error: status code is not in 2XX range")
            print(response.text)
            st.exception(f"status code is not in 2XX range. {response.status_code} \n {response.text}")
        ...
        st.exception("Exception happened in generate_name:", err)
